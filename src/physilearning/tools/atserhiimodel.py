...
class ODEModel:
    ...
    def __init__(
            ...
            params: dict =  {'r_s': 0.03, 'r_r': 0.03},
            consts: dict = {'Delta_s': 0.15},
            tmin: int = 0,
            tmax: int = 100,
            dt: float = 0.1,
            treatment_schedule: list or np.array = None,
            time: list or np.array = None,
            theta: list = [0.03, 0.03],
            const_values = [0.15],
    ) -> None:
        self.rhs = self.LV
        self.y0 = y0
    ...
    def LV(self, t, X, theta):
        ...
        list_of_all_parameters = ['r_s', 'r_r', 'Delta_s']
        ...
        # equations
        dx_dt = s*prm['r_s']*(1-self.treatment*prm['Delta_s'])
        if s <= 0:
            dx_dt = 0
        dy_dt = r*prm['r_r']
