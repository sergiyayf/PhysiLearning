...
def run_simulation(initial_guess=[1,2,3], initial_condition=[0.99,0.01], iterations=6):
    #growth_rate_sus, growth_rate_res, treat_sus = initial_guess
    competition, ramp_up, ramp_down = initial_guess
    ...
    evaluation.env.env.env_specific_params['ramp_time_up'] = ramp_up
    evaluation.env.env.env_specific_params['ramp_time_down'] = ramp_down
...
def minimization_function(initial_guess):
    ...
    experiment_replicas = [f'{base}/Plate_1/Results/Results_G418_a2a4_C{i}_Pulse-C{i}_seg.txt' for i in range(2,6)] + \
                          [f'{base}/Plate_4/Results/Results_G418_a2a4_B{i}_Pulse-B{i}_seg.txt' for i in range(1,7)]
    for path in experiment_replicas:
        sus_exp, res_exp = get_data(path)
        print(path)
        #print(f"Sus exp {sus_exp}, Res exp {res_exp}")
        sus, res = run_simulation(initial_guess, initial_condition=[sus_exp[0], res_exp[0]], iterations=len(sus_exp))
...
def fit_simulation():
    ...
    initial_guess = [6.2, 100.0, 2000.0]
    bounds = [(0.01, 100.0), (0.0, 10000.0), (0.0,10000.0)]  # bounds for growth rates

    def callback_func(xk):
        print(xk)

    result = opt.minimize(minimization_function, np.array(initial_guess), method='Nelder-Mead', callback=callback_func,
                          options={'disp': True, 'maxiter': 2}, bounds=bounds)
...
def visualize_results(opt_params):
    base = '/home/saif/Projects/PhysiLearning/data/experimental_data_elias/2025_08_CoreSeed_Large'
    experiment_replicas = [f'{base}/Plate_1/Results/Results_G418_a2a4_C{i}_Pulse-C{i}_seg.txt' for i in range(2, 3)]


    for path in experiment_replicas:
        fig, ax = plt.subplots(figsize=(10, 6))
        sus_exp, res_exp = get_data(path)
        sus_sim, res_sim = run_simulation(opt_params, initial_condition=[sus_exp[0], res_exp[0]], iterations=len(sus_exp))
