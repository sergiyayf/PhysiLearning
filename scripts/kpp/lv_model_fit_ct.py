...
def run_simulation(initial_guess=[1,2,3], initial_condition=[0.99,0.01], iterations=6):
    ...
    growth_rate_res, death_treat_sus = initial_guess
    ...
    evaluation.env.env.growth_rate[1] = growth_rate_res
...
def minimization_function(initial_guess):
    ...
    experiment_replicas = [f'{base}/Plate_3/Results/Results_G418_a2a4_B{i}_CT-B{i}.txt' for i in range(2,7)] + \
                          [f'{base}/Plate_5/Results/Results_G418_a2a4_C{i}_CT-C{i}.txt' for i in range(1,7)] + \
                          [f'{base}/Plate_5/Results/Results_G418_a2a4_D4_CT-D4.txt']
...
def fit_simulation():
    ...
    initial_guess = [0.19, 0.08]  # growth_rate_res, death_treat_sus
    bounds = [(0.01, 0.9), (0.001, 0.9)]  # bounds for growth_rate_res and death_treat_sus

    def callback_func(xk):
        print(xk)

    result = opt.minimize(minimization_function, np.array(initial_guess), method='Nelder-Mead', callback=callback_func,
                          options={'disp': True, 'maxiter': 2100}, bounds=bounds)
