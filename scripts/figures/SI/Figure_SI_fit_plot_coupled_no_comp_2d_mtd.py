...
def plot_finals():
    ...
    title = eq + "\n" + eq2 + "\n" + r"$r_S = 0.087, r_R = 0.211, K = 2.53, D_S = 4.862$" + r" fit: $r_r$"
    # put the equation on the top as title
    ax.set_title(title, fontsize=8)
    ax.set_xlabel('Time to progression')
    ax.set_ylabel('Normalized cell number')
    fig.savefig(
        r'/home/saif/Projects/PhysiLearning/data/si_figure_panels/coupled_no_comp_mtd_fit.svg',
        transparent=True)
...
if __name__ == '__main__':
    ...
    trace = az.from_json('./../../../data/SI_data/no_comp_2D_patient_x_mtd_LV_inference_Data_1_33_threshold.json')
    plot_finals()
    plt.show()
