...
if __name__ == '__main__':
    ...
    consts_fit = {'Delta_s': 2.60, 'K': 42100, 'r_s': 0.116}
    params_fit = {'r_r': 0.227}
    sigmas = [0.009]
    iteration = 1
    accuracy = 0.0
    tune_draws = 1000
    ...
    while accuracy < 0.95:
        ...
        with pm.Model() as model:
            ...
            r_r = pm.TruncatedNormal("r_r", mu=theta_fit[0], sigma=sigmas[0], initval=theta_fit[0], lower=1.e-3, upper=1)
    ...
    with model:
        trace_DEM = pm.sample(step=[pm.DEMetropolis(vars_list)], tune=2 * draws, draws=draws, chains=chains, cores=16)
    trace = trace_DEM
    trace.to_json('./../../../data/SI_data/elias_lv_fit_02_D7.json')
    plot_finals()
    plt.show()
