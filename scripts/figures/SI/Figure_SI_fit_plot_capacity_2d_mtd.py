...
def plot_finals():
    ...
    title = eq + "\n" + eq2 + "\n" + r"$r_S = 0.268, r_R = 0.248, K = 1.423, D_S = 5.55$" + r" fit: $r_r$"
    # put the equation on the top as title
    ax.set_title(title, fontsize=8)
    ax.set_xlabel('Time to progression')
    ax.set_ylabel('Normalized cell number')
    fig.savefig(
        r'/home/saif/Projects/PhysiLearning/data/si_figure_panels/capacity_mtd_fit.svg',
        transparent=True)
...
if __name__ == '__main__':
    ...
    trace = az.from_json('./../../../data/SI_data/capacity_2D_patient_x_mtd_LV_inference_Data_1_33_threshold.json')
    plot_finals()
    plt.show()
