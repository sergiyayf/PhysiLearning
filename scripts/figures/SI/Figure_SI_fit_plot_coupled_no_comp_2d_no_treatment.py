...
def plot_finals():
    ...
    title = eq + "\n" + eq2 + "\n" + r"$r_S = 0.087, r_R = 0.211, K = 2.53, D_S = 4.862$" + r" fit: $r_S$"
    # put the equation on the top as title
    ax.set_title(title, fontsize=8)
    ax.set_xlabel('Time to progression')
    ax.set_ylabel('Normalized cell number')
    fig.savefig(r'/home/saif/Projects/PhysiLearning/data/si_figure_panels/coupled_no_comp_no_treatment_fit.svg', transparent = True)
...
if __name__ == '__main__':
    ...
    consts_fit = {'Delta_r': 0.0, 'delta_r': 0.01, 'delta_s': 0.01,
                  'r_r': 0.211, 'K': 1.27, 'r_s': 0.087, 'c_s': 1.0, 'c_r': 1.0, 'Delta_s': 4.862}
    params_fit = {}
    sigmas = [0.001]
    sigmas = [0.004]

    trace = az.from_json('./../../../data/SI_data/no_comp_2D_patient_x_no_treatment_LV_inference_Data_1_33_threshold.json')
    plot_finals()
    plt.show()
