...
if __name__ == '__main__':
    ...
    consts_fit = {'Delta_r': 0.0, 'delta_r': 0.01, 'delta_s': 0.01,
                  'r_r': 0.211, 'c_s': 1.733, 'c_r': 1.0, 'Delta_s': 0.142, 'K': 1.27}
    ...
    with model:
        trace_DEM = pm.sample(step=[pm.DEMetropolis(vars_list)], tune=2 * draws, draws=draws, chains=chains, cores=16)
    trace = trace_DEM
    trace.to_json('./../../../data/SI_data/fixed_treatment_2D_patient_x_no_treatment_LV_inference_Data_1_33_threshold.json')
    plot_finals()
    plt.show()