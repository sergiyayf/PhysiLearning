...
def plot_finals():
    ...
    eq = (r"$\frac{dS}{dt} = S \cdot (r_S \cdot (1 - \frac{S + R}{K} ) - \delta_S) - D_S $")
    eq2 = (r"$\frac{dR}{dt} = R \cdot (r_R \cdot (1 - \frac{S c_S + R}{K} ) - \delta_R) $")
    title = eq + "\n" + eq2 + "\n" + r"$r_S = 0.087, r_R = 0.211, K = 2.53, c_S = 1.732, D_S = 0.142$" + r" fit $r_S$"
    # put the equation on the top as title
    ax.set_title(title, fontsize=8)
    ax.set_xlabel('Time to progression')
    ax.set_ylabel('Normalized cell number')
    fig.savefig(r'/home/saif/Projects/PhysiLearning/data/si_figure_panels/fixed_treatment_no_treatment_fit.svg', transparent = True)
...
if __name__ == '__main__':
    ...
    consts_fit = {'Delta_r': 0.0, 'delta_r': 0.01, 'delta_s': 0.01,
                  'r_r': 0.211, 'r_s': 0.087, 'c_r': 1.0, 'K': 1.27}
    params_fit = {'Delta_s': 0.142, 'c_s': 1.732}
    sigmas = [0.001, 0.01]
    sigmas = [0.004]

    trace = az.from_json('./../../../data/SI_data/fixed_treatment_2D_patient_x_at100_LV_inference_Data_1_33_threshold.json')
