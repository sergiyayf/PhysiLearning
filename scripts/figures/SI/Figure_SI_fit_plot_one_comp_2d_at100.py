...
def plot_finals():
    ...
    title = eq + "\n" + eq2 + "\n" + r"$r_S = 0.087, r_R = 0.215, K = 2.53, c_S = 1.733, D_S = 5.707$" + r" fit $c_S, D_S$"
    # put the equation on the top as title
    ax.set_title(title, fontsize=8)
    ax.set_xlabel('Time to progression')
    ax.set_ylabel('Normalized cell number')
    fig.savefig(
        r'/home/saif/Projects/PhysiLearning/data/si_figure_panels/one_comp_at100_fit.svg',
        transparent=True)
...
if __name__ == '__main__':
    ...
    consts_fit = {'Delta_r': 0.0, 'delta_r': 0.01, 'delta_s': 0.01,
                  'r_r': 0.215, 'c_s': 1.733, 'c_r': 1.0, 'Delta_s': 5.707, 'K': 1.27, 'r_s': 0.087}
    params_fit = {}
    sigmas = [0.001]
    sigmas = [0.004]

    trace = az.from_json('./../../../data/SI_data/one_comp_2D_patient_x_at100_LV_inference_Data_1_33_threshold.json')
