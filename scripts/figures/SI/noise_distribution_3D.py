...
for time in range(1,6):
    for i in range(100):
        df = pd.read_hdf('/home/saif/Projects/PhysiLearning/data/3D_benchmarks/p62/p62_no_treat/p62_no_treat_all.h5', key=f'run_{i}')
...
fig, ax = plt.subplots()
sns.histplot(type_0-np.mean(type_0), kde=True, label = 'Data', color = 'blue', ax=ax, bins=15)
plt.show()
...
y = truncnorm.pdf(x, -0.01/std, 0.01/std, loc=mean, scale=std)
...
mean = 0
std2 = 0.0025
y = 1/(std2 * np.sqrt(2 * np.pi)) * np.exp( - (x - mean)**2 / (2 * std2**2))

ax.plot(x, y, label='Normal distribution 2', color = 'magenta')
ax.legend()
plt.show()
