...
if __name__ == '__main__':
    ...
    consts_fit = {'Delta_r': 0.0, 'delta_r': 0.01, 'delta_s': 0.01,
                  'r_r': 0.248, 'r_s': 0.268, 'c_s': 1.0, 'c_r': 1.0}
    params_fit = {'Delta_s': 5.497, 'K': 0.700}
    sigmas = [0.012, 0.005]
    iteration = 1
    accuracy = 0.0
    tune_draws = 1000
    ...
    while accuracy < 0.99:
        ...
        with pm.Model() as model:
            ...
            capacity = pm.TruncatedNormal("K", mu=theta_fit[1], sigma=sigmas[1], lower=0.5, upper=3.0, initval=theta_fit[1])
            sigma = pm.HalfNormal("sigma", 10)

            # Ode solution function
            ode_solution = pytensor_forward_model_matrix(
                pm.math.stack([Delta_s, capacity])
            )
    ...
    with model:
        trace_DEM = pm.sample(step=[pm.DEMetropolis(vars_list)], tune=2 * draws, draws=draws, chains=chains, cores=16)
    trace = trace_DEM
    trace.to_json('./../../../data/SI_data/capacity_2D_patient_x_at100_LV_inference_Data_1_33_threshold.json')
    plot_finals()
    plt.show()
