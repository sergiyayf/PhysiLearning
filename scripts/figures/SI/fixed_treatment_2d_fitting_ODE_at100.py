...
if __name__ == '__main__':
    ...
    while accuracy < 0.99:
        ...
        with pm.Model() as model:
            ...
            D_s = pm.Normal("Delta_s", mu=theta_fit[0], sigma=sigmas[0], initval=theta_fit[0])
            c_s = pm.Normal("c_s", mu=theta_fit[1], sigma=sigmas[1], initval=theta_fit[1])

            sigma = pm.HalfNormal("sigma", 10)

            # Ode solution function
            ode_solution = pytensor_forward_model_matrix(
                pm.math.stack([D_s, c_s])
            )
    ...
    with model:
        trace_DEM = pm.sample(step=[pm.DEMetropolis(vars_list)], tune=2 * draws, draws=draws, chains=chains, cores=16)
    trace = trace_DEM
    trace.to_json('./../../../data/SI_data/fixed_treatment_2D_patient_x_at100_LV_inference_Data_1_33_threshold.json')
    plot_finals()
    plt.show()
