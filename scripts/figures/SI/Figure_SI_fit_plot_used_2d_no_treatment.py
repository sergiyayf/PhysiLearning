...
def plot_finals():
    ...
    eq = (r"$\frac{dS}{dt} = S \cdot (r_S \cdot (1 - \frac{S + c_R R}{K} )(1-D_S) - \delta_S) $")
    eq2 = (r"$\frac{dR}{dt} = R \cdot (r_R \cdot (1 - \frac{S c_S + R}{K} ) - \delta_R) $")
    title = eq + "\n" + eq2 + "\n" + r"$r_S = 0.087, r_R = 0.216, K = 2.53, c_S = 1.772, D_S = 5.837, c_R = 3.408$" + r" fit $r_S$"
    # put the equation on the top as title
    ax.set_title(title, fontsize=8)
    ax.set_xlabel('Time to progression')
    ax.set_ylabel('Normalized cell number')
    fig.savefig(r'/home/saif/Projects/PhysiLearning/data/si_figure_panels/used_no_treatment_fit.svg', transparent = True)
...
if __name__ == '__main__':
    ...
    consts_fit = {'Delta_r': 0.0, 'delta_r': 0.01, 'delta_s': 0.01,
                  'r_r': 0.216, 'K': 1.27, 'r_s': 0.087, 'c_s': 1.772, 'c_r': 3.408, 'Delta_s': 5.837}
    params_fit = {}
    sigmas = [0.001]
    sigmas = [0.004]

    trace = az.from_json('./../../../data/SI_data/2D_patient_x_no_treatment_LV_inference_Data_1_33_threshold.json')
    plot_finals()
    plt.show()
