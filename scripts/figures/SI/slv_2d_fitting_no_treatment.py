...
import pandas as pd
import numpy as np
from physilearning.tools.abstractedodemodel import ODEModel
import arviz as az
...
if __name__ == '__main__':
    ...
    # normallize to the initial total value
    df['Type 0'] = df['Type 0'] / initial_size
    df['Type 1'] = df['Type 1'] / initial_size
    ...
    consts_fit = {'Delta_r': 0.0, 'delta_r': 0.01, 'delta_s': 0.01,
                  'r_r': 0.14, 'c_s': 0.02, 'c_r': 1.0, 'Delta_s': 5.841, 'K': 2.53}
    params_fit = {'r_s': 0.088}
    sigmas = [0.01]
    iteration = 1
    accuracy = 0.0
    tune_draws = 100
    ...
    trace = trace_DEM
    #trace.to_json('./../../data/SI_data/SLV2D_patient_x_np_treatment_SLV_inference_Data_1_33_threshold.json')
    plot_finals()
    plt.show()
