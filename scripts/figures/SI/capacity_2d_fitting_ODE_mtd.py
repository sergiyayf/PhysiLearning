...
if __name__ == '__main__':
    ...
    consts_fit = {'Delta_r': 0.0, 'delta_r': 0.01, 'delta_s': 0.01,
                  'r_s': 0.268, 'c_s': 1.0, 'c_r': 1.0, 'Delta_s': 5.55, 'K': 0.714}
    params_fit = {'r_r': 0.248}
    sigmas = [0.002]
    iteration = 1
    accuracy = 0.0
    tune_draws = 1000
    ...
    with model:
        trace_DEM = pm.sample(step=[pm.DEMetropolis(vars_list)], tune=2 * draws, draws=draws, chains=chains, cores=16)
    trace = trace_DEM
    trace.to_json('./../../../data/SI_data/capacity_2D_patient_x_mtd_LV_inference_Data_1_33_threshold.json')
    plot_finals()
    plt.show()