...
if __name__ == '__main__':
    ...
    # Get data
    df = pd.read_hdf(
        './../../data/2D_benchmarks/mtd/2d_mtd_all.h5', key='run_3')
    ...
    consts_fit = {'Delta_r': 0.0, 'delta_r': 0.01, 'delta_s': 0.01,
                  'r_s': 0.083, 'c_s': 1.793, 'c_r': 3.318, 'Delta_s': 5.962, 'K': 1.27}
    params_fit = {'r_r': 0.221}
    sigmas = [0.002]
    iteration = 1
    accuracy = 0.0
    tune_draws = 1000
    ...
    with model:
        trace_DEM = pm.sample(step=[pm.DEMetropolis(vars_list)], tune=2 * draws, draws=draws, chains=chains, cores=16)
    trace = trace_DEM
    trace.to_json('./../../data/SI_data/2D_patient_x_mtd_LV_inference_Data_1_2_threshold.json')
    plot_finals()
    plt.show()