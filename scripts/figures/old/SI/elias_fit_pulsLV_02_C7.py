...
if __name__ == '__main__':
    ...
    consts_fit = {'Delta_s': 2.57, 'r_r': 0.22, 'K': 42000}
    params_fit = {'r_s': 0.116}
    sigmas = [0.001]
    iteration = 1
    accuracy = 0.0
    tune_draws = 1000
    ...
    while accuracy < 0.99:
        ...
        with pm.Model() as model:
            ...
            r_s = pm.TruncatedNormal("r_s", mu=theta_fit[0], sigma=sigmas[0], initval=theta_fit[0], lower=1.e-3, upper=1)
    ...
    with pm.Model() as model:
        # Priors
        #K = pm.TruncatedNormal("K", mu=theta_fit[0], sigma=sigmas[0], initval=theta_fit[0], lower=10000, upper=120000)
        r_s = pm.TruncatedNormal("r_s", mu=theta_fit[0], sigma=sigmas[0], initval=theta_fit[0], lower=1.e-3, upper=16)
        sigma = pm.HalfNormal("sigma", 10)
    ...
    draws = final_draws
    with model:
        trace_DEM = pm.sample(step=[pm.DEMetropolis(vars_list)], tune=2 * draws, draws=draws, chains=chains, cores=1)
    trace = trace_DEM
    trace.to_json('./../../../data/SI_data/elias_lv_fit_02_r_s_C7.json')
    plot_finals()
    plt.show()
