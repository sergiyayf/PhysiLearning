...
if __name__ == '__main__':
    ...
    #           ]
    fname = f'./pcdl_data_job_15040854_port_0.h5'
    fnames = [fname]
    ...
    for i in range(0, 7): # 1,7 worked
        for j in range(2, 52):
            ...
            df_velocity = bin_cells(df_velocity, bin_size=bn)
    ...
    rads = [x * bn for x in range(0, len(mean_projections))]
    rads = np.array(rads)
    ...
    fig, ax = plt.subplots()
    plot_radial_velocity(ax, mean_projections, std_projs, bin_size=bn)
