...
if __name__ == '__main__':
    ...
    df = pd.read_hdf('./Evaluations/SLvEnvEval__s2t5_on_itself1504_s2_t5_l3.h5', key=f'run_6')
    figure_setup(fig, ax, df, save_figure = True, savename = 'fig3_trajectory_SLV_s2t5_lin.pdf')


    fig, ax = plt.subplots(2, 1, figsize=(130 / 72, 85 / 72), constrained_layout=False,
                           sharex=True, gridspec_kw={'height_ratios': [1, 10]})
    df = pd.read_hdf('data/2D_benchmarks/s2_t5_l3/2d_s2_t5_l3_all.h5', key=f'run_7')
    figure_setup(fig, ax, df, save_figure=True, savename='fig3_trajectory_PC_s2t5_lin.pdf')