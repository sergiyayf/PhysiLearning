...
class PC_env(Env):
    ...
    def step(self, action):
        ...
        print("State 0:" +str(self.state[0]))
        ...
        if np.sum(self.state[0:2]) != 0:
            reward = (1000-np.sum(self.state[0:2]))*1e-3 # this means about 60 % of the simulation space is filled 
        else:
            reward = 5
    ...
    def reset(self):
        ...
        self.socket.connect("tcp://localhost:5569")
    ...
    def cleanup(self):
        command = "cd .. && make data-cleanup && exit"
